...
| uav_attack_whelan | autonomy · gnss (+network · c2_link for ping-DoS) | telemetry_sample | adapter done; **run on the real 3-flight live sample** (benign/jam/spoof), 610 events schema-valid; grounds the empirical δ (γ≈1.2–1.4 m/s). Attack intervals still TODO (self-ref calibration) |
| uav_cas | network · mesh | flow | adapter done + fixture-verified; **real stat CSV not yet on disk** (absent from all sources this session — PENDING_ON_DATA.md #1); ts out of scope |
...
Five of six datasets are now ingested on REAL data and schema-validate
(EW-Bench 93,600 · Whelan 3-flight · UAVIDS 122,171 · HCRL 10 scenarios ·
DATAMUt sim); only the real UAV-CAS stat CSV is missing (absent from all
sources this session — `PENDING_ON_DATA.md`). The certified-regime question is
resolved AND the decisive real result is in (`docs/certified_regime_analysis.md`
+ its 2026-07-29 ADDENDUM): the empirical Whelan δ mapping (γ≈1.2–1.4 m/s,
~11× tighter than kinematic v_max) brings **θ=0.25 s INSIDE the certified
window** at every corridor margin → Paper A takes the "widened window" framing
(`paper/abstract_A_widened.tex`; narrow fallback drafted), pending cruise-regime
confirmation. Certificate radius locked at the measured local **0.153** (L=1.18).
Both papers reconciled to the clean campaign (`paper/paperA.tex`, `paperC.tex`).
One-command real run: `experiments/run_real_corpus.sh`. Layout for staging any
dataset: `docs/data_staging_layout.md`. Remaining data-gated items (only these):
`PENDING_ON_DATA.md` — real UAV-CAS stat, Whelan attack intervals + cruise
flights, TEXBAT bridge re-calibration, PAC-Bayes KL, HCRL map confirmation.
Keep everything schema-valid and host-agnostic.
