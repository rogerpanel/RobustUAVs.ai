...
$x_{\mathrm{att}}(t)$ the trajectory under the perturbed input $u(t)+\delta u(t)$,
both started from the same initial state $x_{\mathrm{nom}}(0)=x_{\mathrm{att}}(0)$.
Write $e(t)=x_{\mathrm{att}}(t)-x_{\mathrm{nom}}(t)$ for their deviation. From
\eqref{eq:ode},
\[
\dot e(t)=F\big(x_{\mathrm{att}},u+\delta u\big)-F\big(x_{\mathrm{nom}},u\big).
\]
Because $F$ is $L$-Lipschitz in its arguments over the operating region,
$\Norm{\dot e(t)}\le L\,\Norm{e(t)}+L\,\Norm{\delta u(t)}$. Integrating and using
$e(0)=0$,
\[
\Norm{e(t)}\;\le\;L\!\int_0^t\!\Norm{e(\tau)}\,d\tau
\;+\;L\!\int_0^t\!\Norm{\delta u(\tau)}\,d\tau .
\]
Applying the integral form of Grönwall's inequality~\cite{gronwall1919} to the
non-decreasing forcing term $g(t)=L\!\int_0^t\!\Norm{\delta u}\,d\tau$ yields
$\Norm{e(t)}\le g(t)\,e^{Lt}$, and bounding the input by
$\Norm{\delta u(\tau)}\le\delta(\theta)$ for all $\tau$ gives, at the horizon,
\begin{equation}
\label{eq:tube_derivation}
\Norm{e(T)}\;\le\;\delta(\theta)\,\big(e^{LT}-1\big)\;\le\;\delta(\theta)\,e^{LT},
\end{equation}
which is the tube radius \eqref{eq:tube} (the looser right-hand bound is the one
we report, as it is closed-form and monotone). Now suppose $\rho(\theta)\le m$.
The nominal trajectory lies in the safe corridor by construction, and every point
of the attacked trajectory is within $\rho(\theta)\le m$ of it by
\eqref{eq:tube_derivation}; hence the attacked trajectory never exits the
corridor over $[0,T]$ and the mission completes. Taking the probability of the
event $\{\rho(\theta)\le m\}$ over the mission distribution---the only source of
randomness once $\theta$ is fixed, since $\delta(\theta)$ is a deterministic
bound---gives the floor \eqref{eq:floor}. Finally, composing the three maps
$\theta\!\mapsto\!\Delta(\theta)$ (Lemma~\ref{lem:residual}),
$\Delta\!\mapsto\!\delta$ (\S\ref{sec:interface}), and $\delta\!\mapsto\!\MCR$
(the argument just given) establishes the end-to-end chain \eqref{eq:chain}.
\end{proof}

\begin{proposition}[Soundness]
\label{prop:sound}
The certified floor never overstates the achievable mission-completion rate:
for every detector operating point $\theta$, the true $\MCR$ of any
$D_\theta$-evading attack satisfies $\MCR\ge f(\delta(\theta))$.
\end{proposition}
\begin{proof}
Every step above is an inequality in the conservative direction: the
Lipschitz bound over-estimates the deviation growth, the input bound uses the
worst-case $\delta(\theta)$, and the corridor test is a sufficient (not
necessary) condition for completion. Hence $f(\delta(\theta))$ is a lower bound
on the true completion probability, which is precisely soundness; the empirical
validation of \S\ref{sec:instantiation} confirms the certified floor lies below
the measured rate at every operating point.
\end{proof}
...
operating point flows top-to-bottom through a residual-budget bound
(Lemma~\ref{lem:residual}), an empirically grounded interface mapping
(\S\ref{sec:interface}), a Gr\"onwall trajectory tube, and a corridor-margin
test, arriving at a certified mission-completion floor. The upper two stages are
network-side quantities; the lower two are autonomy-side; the perturbation
$\delta(\theta)$ is the bridge between them.}
\label{fig:chain}
\end{figure}
...
\begin{semilogxaxis}[
  ...
  legend style={at={(0.5,1.1)}, anchor=south, font=\footnotesize, draw=black!30,
...
\begin{loglogaxis}[
  ...
  legend style={at={(0.5,1.1)}, anchor=south, font=\small, draw=black!30},
  grid=both, grid style={black!12},
  tick label style={font=\small},
  label style={font=\small}]
