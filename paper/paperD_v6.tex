...
second of delayed navigation input costs between $0.5$ and $1.4$\,m of position
error on real GPS-jamming and GPS-spoofing flights, an order of magnitude less
...
three flights of one airframe in hover, and that rate varies by $2.6\times$
between the two attack types we can observe. The guarantee is conditional on
...
not: it varies by $2.6\times$ between the two attack types the corpus permits us
to compare (\S\ref{sec:gammastability}). The certificate is therefore evaluated
at the maximum over observed conditions, and the reported spread is a lower
bound on the true variability.
...
constant: it varies by $2.6\times$ between the two attack types we can observe.
\end{enumerate}
...
paper. We test it over the factors the released corpus lets us vary, and we are
explicit about the ones it does not.

Table~\ref{tab:gamma} reports $\gamma$ for each combination of attack type and
error signal available on the calibration flights. The answer is negative:

\begin{quote}
$\gamma$ is not a constant. It varies by $2.6\times$ between GPS jamming
($\approx0.5$\,m/s) and GPS spoofing ($\approx1.4$\,m/s).
\end{quote}

The direction is mechanistically unsurprising, and that it comes out the right
way is a weak check on the measurement. Jamming \emph{removes} information: the
receiver loses lock, the estimator gates the missing fix out and dead-reckons,
and position error accrues at the inertial drift rate. Spoofing \emph{injects}
information: the estimator accepts a fix that is wrong but internally
consistent, and error accrues at the rate the attacker chooses to walk it away.
An attack that supplies a plausible lie should therefore drive error faster than
one that supplies nothing, and it does. By contrast the choice of error signal
matters little ($1.14\times$ between the raw receiver fix and the EKF-filtered
position), which is itself informative: the filter attenuates the injected error
far less than one might hope.

\begin{table}[t]
\centering
\caption{The delay-to-position rate $\gamma$ is attack-type dependent. Measured
on the three calibration flights of the \dataset{UAV Attack Dataset}. The
certificate consumes the maximum over observed conditions, which is the
conservative choice; the spread is a lower bound on $\gamma$'s true variability,
since the release fixes platform, airspeed, flight mode, attack intensity, and
attack duration. Generated by \code{experiments/gamma\_stability.py}.}
\label{tab:gamma}
\small
\renewcommand{\arraystretch}{1.05}
...
Two consequences for how the guarantee must be read.

First, the honest model is $\gamma=\gamma(\text{attack type},\ldots)$ rather
than $\gamma=\text{const}$, and the certificate must therefore be evaluated at
the supremum over the conditions it is claimed to cover. Every certified
quantity in this paper uses $\gamma=1.365$\,m/s, the maximum over the four
measured combinations. Using the jamming rate would widen the certified window
by a factor of $2.6$ and would not be defensible against an adversary free to
choose the attack.

Second, and more importantly, \emph{four combinations on one airframe in hover
is a calibration, not a characterisation}. The release fixes platform, airspeed,
flight mode, attack intensity, and attack duration, so the $2.6\times$ spread we
report is a lower bound on the variability a deployment would encounter. This is
condition~\ref{c:interface} of \S\ref{sec:scope}, and it is the weakest link in
the chain. A campaign varying those five factors is the single highest-value
extension of this work; \S\ref{sec:limitations} treats it as such. Until it
exists, the correct reading of our headline number is ``the certified window is
non-empty under a rate measured in the most adverse condition we could
observe'', not ``the certified window is non-empty''.
...
even there, varying by $2.6\times$ between the two attack types available. The
release fixes five further factors we would need to vary before calling $\gamma$
characterised: \emph{platform}, \emph{airspeed}, \emph{flight mode},
\emph{attack intensity}, and \emph{attack duration}. The reported spread is
...
$\gamma$ already varies by $2.6\times$ with the attack type. What we offer is
