...
\title{From Wire to Flight: A Cross-Layer Benchmark and a Composed\\
Network-to-Navigation Guarantee for Autonomous UAVs}
...
Network intrusion detection catches malicious traffic on the swarm mesh, the
command link, and the on-board bus, but says nothing about the flight; certified
robustness bounds a navigation model's error but assumes the perturbation
arrives from nowhere. Neither answers the question an operator actually asks:
\emph{when an attack slips past the detector, does the aircraft still complete
its mission?} We close the gap from both ends. First, we build the end-to-end
benchmark that makes the question measurable: six datasets spanning the network
and autonomy layers, unified under a schema whose central object is a
\emph{cross-layer pairing}---a network attack window joined to the navigation
degradation it induces, tagged with an auditable flag recording whether that
coupling was measured on one platform, physically modelled, or aligned in
simulation. Second, we prove on exactly those objects a \emph{composition
theorem}: for any attack evading a detector at operating point $\theta$, the
navigation stack retains a certified Mission-Completion-Rate
$\MCR\ge f(\delta(\theta))$. Two measurements make the guarantee sharp rather
than nominal. The residual attack budget \emph{saturates} at the network's
contact-window slack---verified as at most $4.84$\,s of undetected delay over
$15{,}392$ observed hops---and the delay-to-position interface, calibrated on
real GPS-spoofing and jamming flights on the target autopilot family, is an
order of magnitude tighter than the kinematic worst case. Under that measured
interface the detector's operating point falls \emph{inside} a nonempty
certified window, and the composed guarantee is the only configuration with a
nonzero certified floor against an evading attacker, dominating both
single-layer baselines at every operating point (Wilcoxon, Holm-corrected,
$p<10^{-20}$). We release the schema, six validated adapters, the harness, and
the certificate engine as an open artifact in which every reported number
...
outside UAV security needs, with tables of every acronym and symbol.
...
% =====================================================================
This section is written for the reader who reviews security or
machine-learning work but does not specialise in drones.
Table~\ref{tab:acronyms} collects the acronyms and Table~\ref{tab:notation} the
mathematical symbols; both are the single reference for the rest of the paper.
...
exchange corrections and commands over radio links: a C2 link (typically the
MAVLink protocol), a multi-hop mesh in which peers relay each other's traffic
during periodic contact windows---a delay-tolerant network (DTN)---and an
intra-vehicle bus (UAVCAN/DroneCAN) connecting the flight controller to motor
controllers and sensors. An attacker on the mesh can therefore reach the
...
raises $J/S$ until the receiver loses satellite lock; a \emph{spoofer}
...
TWiG & time-window graph (the forwarding-schedule model)\\
UAVCAN & UAV Controller Area Network (intra-vehicle bus)\\
RF / EW & radio frequency / electronic warfare\\
$J/S$ & jamming-to-signal power ratio, in dB\\
IDS / FPR & intrusion detection system / false-positive rate\\
...
MWU / KL & multiplicative-weights update / Kullback--Leibler divergence\\
...
\subsection{Mission completion and the regulatory floor}
We adopt \emph{Mission Completion Rate} (MCR)---the fraction of flights that
reach their objective under attack---as the operational metric, and take
$\MCR\ge0.90$ as an operational reference consistent with the
airworthiness-security posture of DO-326A/ED-202A~\cite{do326a}. Emerging
AI-in-aviation assurance guidance~\cite{ed324} motivates treating a certified
MCR floor as a first-class safety property, which is what
\S\ref{sec:theorem} delivers.
...
\label{sec:related}
% =====================================================================
We organise prior work along the two halves of the paper: what exists on the
data side (corpora and their construction), and what exists on the guarantee
side (bounding a model, a controller, or a mission). The recurring gap is that
each community works within one layer and leaves the interface to assumption.
...
each cover a single sublayer. Notably, models trained on generic benchmarks
misclassify a large fraction of GPS-spoofing events on real autopilot
logs~\cite{islam2025uavids}---direct evidence that the network and navigation
layers cannot be treated as one undifferentiated feature space. FANET security
surveys~\cite{tsao2022survey,ceviz2024survey} catalogue routing attacks but stop
at network effect. On the swarm mesh, \dataset{UAVIDS-2025}~\cite{uavids2025,%
zeng2025uavids} provides $122{,}171$ NS-3 flow records across four attack
classes; on the C2 link, the \dataset{UAV Attack
...
\dataset{HCRL UAVCAN} dataset~\cite{hcrl_uavcan} captures flooding, fuzzing, and
replay. Each is authoritative for its sublayer; none reaches the flight.

\paragraph{Why dataset construction is itself a contribution}
The intrusion-detection community has learned that a benchmark's
\emph{construction} determines the validity of everything built on it. Audits of
the widely used CIC-IDS2017 corpus uncovered labelling errors, flawed flow
construction, and unrealistic temporal separation that materially inflate
reported performance~\cite{engelen2021troubleshooting,sharafaldin2018cicids}. A
cross-layer benchmark adds a new failure mode: asserting a causal link between a
network event and a flight degradation that were never observed together. Our
mandatory per-record honesty flag (\S\ref{sec:schema}) exists precisely to bound
that risk, making provenance discipline---not merely scale---a design
requirement.

\paragraph{Positioning against the closest prior art}
The nearest neighbour in ambition is \dataset{UAV-CAS}, a large
AERPAW-calibrated swarm dataset that is the first to include
...
remains within the network layer, and its authors explicitly list GNSS spoofing
and jamming as future work. Our contribution is complementary: rather than
deepen one layer we \emph{span} both and make the coupling the unit of analysis.
Where \dataset{UAV-CAS} asks how well a collaborative mesh attack can be
detected, we ask what that attack, detected or missed, does to the flight.
...
deterministic bounds that are known to be loose globally. Neural
ODEs~\cite{chen2018neuralode} admit Gr\"onwall-type robustness
arguments~\cite{yan2020robustnode}, the continuous-time analogue we use. These
bound a \emph{model's} output. Lifting to sequential decision-making, CROP
certifies a cumulative-reward lower bound for policies under state
...
network-controlled systems~\cite{ivanov2019verisig,tran2020nnv}; control-theoretic
work provides safety under sensor false-data injection~\cite{li2022lqg}. CROP is
the closest analogue to a certified mission floor, but it certifies a policy
against a \emph{given} perturbation ball---it does not connect that ball to a
network-security operating point.

\paragraph{Secure state estimation, runtime assurance, delay attacks}
A parallel control-theoretic line asks how much sensor corruption an estimator
can provably tolerate: exact recovery is possible below a sensor-corruption
threshold~\cite{fawzi2014secure}, the combinatorial search is practical with
SMT solvers~\cite{shoukry2017smt}, and recent surveys catalogue the
field~\cite{ding2021survey}. These bound estimation error given \emph{which}
sensors are compromised; they do not model a detector whose threshold shapes
what the attacker can inject in the first place---the premise our
Lemma~\ref{lem:residual} supplies. The Simplex architecture and its
...
monitor fires; our certified window is exactly the region in which the
high-performance policy may run, and we supply the quantitative boundary such
architectures otherwise hand-tune. Finally, the effect of injected delay on
feedback stability is well studied, with detectors based on state-aware
inference or divergence tests~\cite{lou2019timedelay,etd2023uav,datamut2026};
these quantify \emph{whether} a delay attack is dangerous or detectable, but
none carries the surviving delay through to a certified mission floor.

\paragraph{Composing guarantees---the crux}
The closest prior art composes certificates \emph{within} an ML pipeline.
\cite{sheikholeslami2021provably} jointly certifies a classifier with a
detection class, so an input is provably flagged or correctly
classified---structurally our evade-or-bound dichotomy, but single-layer.
\cite{yang2022sensing} decomposes end-to-end certified robustness of a two-stage
sensing$\to$reasoning pipeline, the strongest template for chained
certification, but stays within perception. Assume-guarantee and contract-based
CPS design~\cite{pacti2024} give the compositional vocabulary but not
ML-robustness certificates. None chains a \emph{network-intrusion-detection}
guarantee to a \emph{navigation} certificate. That is our gap, and closing it
requires the benchmark and the theorem together.
...
\textbf{CrossLayerPairing} realises the tuple $P$ of \eqref{eq:pairing}: it
...
mission-defined safe corridor for all $t\in[0,T]$; MCR is the probability of
completion over the mission distribution. An attack perturbs the input,
$u\mapsto u+\delta u$.
...
trajectory never exits the corridor over $[0,T]$ and the mission completes.
Taking the probability of $\{\rho(\theta)\le m\}$ over the mission
distribution---the only source of randomness once $\theta$ is fixed, since
$\delta(\theta)$ is a deterministic bound---gives \eqref{eq:floor}. Composing
...
$e^{LT}$ in \eqref{eq:tube} is the core certificate's Achilles heel: for large
...
\emph{Which certificate governs when.} The four do not compete for the same
operating point; each binds in a different regime and the headline floor is
their pointwise minimum. For the time-delay class this paper foregrounds, the
perturbation is a bounded deterministic state staleness, so Lipschitz--Gr\"onwall
is active and sets the certified window of \S\ref{sec:eval}. Randomized
smoothing takes over when the budget is known only statistically---a jamming
intensity characterised by a distribution rather than a hard bound. PAC-Bayes
binds when a floor certified on the training mission mix must survive unseen
missions. MWU regret binds over the \emph{time} axis, when the jammer adapts
across a mission. Because their binding regimes are disjoint, the minimum is
never vacuous for a reason another certificate could have prevented.
...
confirming the saturation predicted by Lemma~\ref{lem:residual}.
