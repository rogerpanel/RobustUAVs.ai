...
\title{From Wire to Flight: An End-to-End Framework for Network-to-Navigation UAV Security}
...
This paper makes that question precise and then answers it with a proof. We
propose a two-layer schema whose unit of analysis is the \emph{cross-layer
pairing}: an interval during which a network attack is active, joined to the
interval of degraded navigation that it causes, and tagged with how strongly
that causal link is evidenced. We instantiate the schema over six public
datasets and prove a \emph{composition theorem}: for an attacker who delays
relayed messages by just less than the detector's threshold, the fraction of
missions that still complete is bounded from below by a quantity computable from
that threshold alone. Two measurements make the bound useful rather than
vacuous. First, the delay an attacker can hide does not grow without limit: it
saturates at $4.84$\,s across $15{,}392$ observed relay hops. Second,
calibration on real GPS-spoofing and jamming flights shows that one second of
delayed navigation input costs about $1.4$\,m of position error, an order of
magnitude less than a worst-case kinematic argument assumes. Under that
calibration a detector setting used in practice admits a non-vacuous guarantee,
whereas neither the network defence nor the navigation defence alone certifies
any non-zero floor. Schema, dataset adapters, and certificate engine are
released as an open artifact.
\end{abstract}
...
carries a network-security threshold through to a navigation-safety floor.
...
concurrent rather than alternative. Consider a ten-minute delivery sortie flown
by a model trained on last season's mission mix, through a corridor in which a
compromised relay delays peer corrections and a ground jammer varies its
transmit power. Four claims hold at once, and each is about a different object.
The Lipschitz--Gr\"onwall tube bounds the trajectory deviation caused by the
\emph{deterministic, bounded} part of the disturbance, the undetected forwarding
delay $\Delta(\theta)$; it alone yields a per-instant geometric envelope, so it
alone discharges the corridor test of Eq.~\eqref{eq:mcr}. Randomized smoothing
bounds the \emph{stochastic} part, the jammer's residual perturbation of the
receiver feature vector, and its claim concerns the model's \emph{decision} at a
point rather than the trajectory. PAC-Bayes bounds the gap between the risk this
model showed on its training mission mix and its risk on \emph{this} sortie: a
statement about the model, constant across the flight. MWU regret bounds the
cumulative loss of the defender's \emph{policy switching} against an adapting
jammer, a statement about the time axis that is vacuous at any single instant.
...
$\gamma>\gamma_{\mathrm{req}}=6.14$\,m/s at a $10$\,m corridor, a
four- to five-fold increase over what we observe, the operating point would
leave the certified window and the honest framing would revert to the narrow
...
increase would require a sustained heading error of roughly $18^\circ$, but an
argument is not a measurement, and a larger cruise-inclusive flight campaign
...
proved, on exactly its objects, the guarantee that the coupling enables: a two-layer schema whose unit
...
Three findings survive scrutiny. The composed guarantee is the only
configuration certifying a nonzero mission floor against a detector-evading
attacker, dominating both single-layer baselines at every operating point. The
empirically calibrated interface, an order of magnitude tighter than the
kinematic worst case, brings the detector's real operating point inside the
...
the interface, not the theorem, decides the outcome. UAV network security and
UAV autonomy security are thus not two problems but two ends of one
certificate.
