...
% "under attack". Shortened to two typeset lines.
\title{From Wire to Flight: Composing Network Intrusion Detection
with Certified UAV Mission Safety}

% AUTHOR BLOCK
% K. Soltani is credited in the Acknowledgements rather than the author list:
% his contribution to this work is the released DATAMUt detector source and
% clarification of its operating point, not manuscript content. Restoring him
% as an author is a one-line change if that judgement is revised.
\ifanon
...
\begin{abstract}
Unmanned aerial vehicle (UAV) security is studied as two disjoint problems.
Network intrusion detection bounds what reaches the aircraft but says nothing
about the flight; certified robustness bounds a navigation model's response to
an input perturbation but treats the origin of that perturbation as exogenous.
Neither answers the question that determines airworthiness: whether an attack
that evades the detector still lets the mission complete.
This paper makes that question provable. A two-layer schema promotes the
\emph{cross-layer pairing} (a network attack window joined to the navigation
degradation it induces) to a first-class object carrying a mandatory provenance
flag, and an instrumented corpus of six datasets supplies the quantities it
consumes. On these objects we prove a \emph{composition theorem} that lower
bounds the certified Mission-Completion-Rate (MCR) of any time-delay forwarding
attack evading a detector at a given operating point. Two measurements make the
bound sharp rather than nominal: the residual attack budget saturates at the
network's contact-window slack, at most $4.84$\,s of undetected delay over
$15{,}392$ observed hops, and the delay-to-position interface, calibrated on
real GPS-spoofing and jamming flights, is an order of magnitude tighter than the
kinematic worst case. Under that interface the operating point of a deployed
detector lies inside a nonempty certified window, and the composed configuration
is the only one certifying a nonzero mission floor against an evading attacker.
Schema, adapters, harness, and certificate engine are released as an artifact in
which every reported number carries a provenance tag.
\end{abstract}
...
breaking cryptography but by degrading the physics: a jammer raises the received
noise floor until the satellite-navigation solution collapses and the mission
fails~\cite{kassas2022jammer}. A forwarding attack in the mesh does not
exfiltrate data; it delays a correction until the aircraft is acting on stale
state~\cite{datamut2026,lou2019timedelay}. In every case the \emph{network}
event and the \emph{navigation} outcome are two ends of one causal chain, and
yet neither the datasets nor the guarantees represent that chain.

The consequence is a claim neither community can currently substantiate. A UAV
intrusion detector can be shown to flag delayed forwarding events or spoofed
navigation messages at high recall, but the operationally decisive
claim, that a detector tuned to a given false-alarm budget keeps aircraft
completing their missions, cannot be evaluated at all, because no public dataset
records both a network event and the flight it degrades. The certified-robustness
literature is symmetric in its silence: it establishes that a navigation model is
provably stable for input perturbations up to some radius $R$, which is a
statement about a model rather than about a flight, and which is silent on where
a radius-$R$ perturbation originates or how often an adversary can produce one.

Closing this gap requires work at both ends simultaneously, because the two
halves are mutually dependent: a guarantee that spans the layers needs objects
that record the coupling, and a corpus that records the coupling needs a
guarantee to show what those objects are for. Accordingly, we unify six
datasets, four established network-layer corpora and two navigation-layer
sources, under a schema whose unit of analysis is neither a packet nor a flight
but a \emph{cross-layer pairing}: a network attack window joined to the autonomy
degradation window it induces. On exactly those objects we then prove that a
detector's operating point determines a lower bound on the fraction of missions
that still complete when the adversary is assumed to evade detection by
construction. To our knowledge this is the first guarantee that carries a
network-security threshold through to a navigation-safety floor. Concretely, the
...
\S\ref{sec:discussion}--\S\ref{sec:conclusion} discuss, bound, and conclude. The
body is self-contained: acronyms are defined at first use, and readers new to
either UAV autonomy or certified robustness may find the consolidated system
model, terminology, and symbol table of Appendix~\ref{sec:background} a
...
not an artefact of our analysis but the standard unit of operational approval
...
concurrent rather than alternative, which is clearest with one flight held
fixed: a ten-minute delivery sortie, flown by a model trained on last season's
mission mix, through a corridor in which a compromised relay delays peer
corrections and a ground jammer varies its transmit power. Four claims hold
simultaneously, about four different objects. (i) The Lipschitz--Gr\"onwall tube
bounds the trajectory deviation produced by the \emph{deterministic, bounded}
component of the disturbance (the undetected forwarding delay $\Delta(\theta)$);
it alone yields a per-instant geometric envelope, so it alone discharges the
corridor test of Eq.~\eqref{eq:mcr}. (ii) Randomized smoothing bounds the
\emph{stochastic} component: the jammer's residual perturbation of the receiver
feature vector admits only a distribution, and the resulting claim concerns the
model's \emph{decision} at a point, not the trajectory. (iii) PAC-Bayes bounds
the gap between the risk this model exhibited on its training mission mix and
the risk on \emph{this} sortie, a statement about the model that is constant
across the flight. (iv) MWU regret bounds the cumulative loss of the defender's
\emph{policy switching} against an adapting jammer, a statement about the time
axis that is vacuous at any single instant.
...
\emph{Two methodological findings we surface rather than bury.} First, our
initial operating curve was contaminated by an accumulation bug in the sweep
harness, in which each operating point inadvertently averaged all previous ones;
the tell was recall \emph{rising} with the threshold, which is impossible for a
threshold detector. The clean multi-seed campaign of Fig.~\ref{fig:opcurve}
replaces it. Second, the bus dataset's attack labels are not machine-readable
per scenario, so rather than guess we derived them from frame statistics and
flagged the derivation for confirmation. Both episodes argue \emph{for} the
provenance discipline the schema enforces.
...
benchmark that makes the coupling representable and proved, on exactly its
objects, the guarantee that the coupling enables: a two-layer schema whose unit
